...
const InquiryMobile = window.intlTelInput(phoneInputField, {
  utilsScript:
...
document.getElementById("send_form").addEventListener("click", function () {
  ...
  let recaptcha = document.getElementById("recaptcha-token").value;
  ...
  if (
    ...
    req_params.append("date", date);
    req_params.append("people", people);
    req_params.append("g-recaptcha-response", recaptcha);

    $.ajax({
      type: "POST",
      url: "https://helloindiatour.com/manager/plantour.php",
      data: req_params,
      processData: false,
      contentType: false,
